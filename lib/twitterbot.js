    ...
    client.post('statuses/update', Object.assign({}, params, { status }), (err, data, res) => {
      if (err) {
        ...
        if (0 !== nextTweet.length) {
          // Set @reply params, if empty
          if ({}.hasOwnProperty.call(params, 'in_reply_to_status_id')) {
            params.in_reply_to_status_id = data.id_str;
            params.in_reply_to_screen_name = data.user.screen_name;
          }

          setTimeout(tweet(nextTweet.join(' '), params, (err) => {
            if (err) {
